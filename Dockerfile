...
WORKDIR /go/src/github.com/glours/docker-lint
...
# docker-lint
####
FROM scratch AS docker-lint
COPY --from=build /go/src/github.com/glours/docker-lint/bin/* /
...
COPY --from=cross-build /go/src/github.com/glours/docker-lint/dist /
...
# install docker-lint plugin
COPY --from=cross-build /go/src/github.com/glours/docker-lint/dist/docker-lint_${TARGETOS}_${TARGETARCH} ./bin/docker-lint_${TARGETOS}_${TARGETARCH}
RUN chmod +x ./bin/docker-lint_${TARGETOS}_${TARGETARCH}
